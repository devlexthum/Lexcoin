...
export BUILDER_IMAGE_NAME="lex-builder-$BUILD_TARGET-$JOB_NUMBER"
...
export GOAL="install"
export SDK_URL=${SDK_URL:-https://lexthum.com/depends-sources/sdks}
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export CHECK_DOC=1
  export LEX_CONFIG="--enable-glibc-back-compat --enable-reduce-exports"
elif [ "$BUILD_TARGET" = "win32" ]; then
  export HOST=i686-w64-mingw32
  export DPKG_ADD_ARCH="i386"
  export PACKAGES="python3 nsis g++-mingw-w64-i686 wine-stable wine32 bc"
  export LEX_CONFIG="--enable-gui --enable-reduce-exports --disable-miner"
  ...
  export LEX_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-stacktraces LDFLAGS=-static-libstdc++"
  export USE_SHELL="/lex/lexcoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export LEX_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-stacktraces"
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_LOOX_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export LEX_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports"
  export PYZMQ=true
elif [ "$BUILD_TARGET" = "mac" ]; then
  export HOST=x86_64-apple-darwin11
  export PACKAGES="cmake imagemagick libcap-dev librsvg2-lex libz-dev libbz2-dev libtiff-tools"
  export LEX_CONFIG="--enable-gui --enable-reduce-exports --disable-miner"
