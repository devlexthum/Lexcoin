...
import (
    ...
    "github.com/devlexthum/Lexcoin/go-address"
    "github.com/devlexthum/Lexcoin/go-bitfield"
    datatransfer "github.com/devlexthum/Lexcoin/go-data-transfer/v2"
    "github.com/devlexthum/Lexcoin/go-fil-markets/retrievalmarket"
    "github.com/devlexthum/Lexcoin/go-fil-markets/storagemarket"
    "github.com/devlexthum/Lexcoin/go-jsonrpc"
    "github.com/devlexthum/Lexcoin/go-state-types/abi"
    "github.com/devlexthum/Lexcoin/go-state-types/big"
    "github.com/devlexthum/Lexcoin/go-state-types/builtin/v8/paych"
   "github.com/devlexthum/Lexcoingo-state-types/builtin/v9/market"
    "github.com/devlexthum/Lexcoin/go-state-types/builtin/v9/miner"
    verifregtypes "github.com/devlexthum/Lexcoin/go-state-types/builtin/v9/verifreg"
    "github.com/devlexthum/Lexcoin/go-state-types/crypto"
   "github.com/devlexthum/Lexcoin/go-state-types/dline"
    abinetwork "github.com/devlexthum/Lexcoin-project/go-state-types/network"

    apitypes "github.com/devlexthum/Lexcoin/lotus/api/types"
    "github.com/devlexthum/Lexcoin/lotus/chain/actors/builtin"
    lminer "github.com/devlexthum/Lexcoin_project/lotus/chain/actors/builtin/miner"
    "github.com/devlexthum/Lexcoin/lotus/chain/actors/builtin/power"
   "github.com/devlexthum/Lexcoin-project/lotus/chain/types"
   "github.com/devlexthum/Lexcoin/lotus/chain/types/lextypes"
   "github.com/devlexthum/Lexcoin/lotus/node/modules/dtypes"
   "github.com/devlexthum/Lexcoin/lotus/node/repo/imports"
)
...
//
// NOTE: This is the V1 (Unstable) API - to add methods to the V.0.2 (Stable) API
// you'll have to add those methods to interfaces in `api/v.0.2api`
//
...
// FullNode API is a low-level interface to the Lexcoin network full node
type FullNode interface {
    Common
    Net

    // MethodGroup: lex_network
    // The lex method group contains methods for interacting with the
    ...
    // genesis and these snapshots can be used to initialize Lexcoin
    // nodes.
    ...
    // StateCirculatingSupply returns the exact circulating supply of Lexcoin at the given tipset.
    // This is not used anywhere in the protocol itself, and is only for external consumption.
    StateCirculatingSupply(context.Context, types.TipSetKey) (abi.TokenAmount, error) //perm:read
    // StateVMCirculatingSupplyInternal returns an approximation of the circulating supply of Lexcoin at the given tipset.
    ...
    // StateGetBeaconEntry returns the beacon entry for the given Lexcoin epoch. If
    ...
    // MethodGroup: sync
    // The sync methods are for interacting with and managing payment channels
    ...
    // MethodGroup: Lex
    // These methods are used for Lexthum-compatible JSON-RPC calls
    //
    // LexAccounts will always return [] since we don't expect Lotus to manage private keys
    LexAccounts(ctx context.Context) ([]lextypes.lexAddress, error) //perm:read
    // LexAddressToLexcoinAddress converts an LexAddress into an f410 Lexcoin Address
    LexAddressToLexcoinAddress(ctx context.Context, lexAddress lextypes.LexAddress) (address.Address, error) //perm:read
    // LexcoinAddressToLexAddress converts an f410 or f0 Lexcoin Address to an LexAddress
    LexcoinAddressToLexAddress(ctx context.Context, lexcoinAddress address.Address) (lextypes.LexAddress, error) //perm:read
    // LexBlockNumber returns the height of the latest (heaviest) TipSet
    LexBlockNumber(ctx context.Context) (lextypes.LexUint64, error) //perm:read
    // LexGetBlockTransactionCountByNumber returns the number of messages in the TipSet
    LexGetBlockTransactionCountByNumber(ctx context.Context, blkNum lextypes.lexUint64) (lextypes.lexUint64, error) //perm:read
    // LexGetBlockTransactionCountByHash returns the number of messages in the TipSet
    LexGetBlockTransactionCountByHash(ctx context.Context, blkHash lextypes.lexHash) (lextypes.lexUint64, error) //perm:read

    LexGetBlockByHash(ctx context.Context, blkHash lextypes.LexHash, fullTxInfo bool) (lextypes.LexBlock, error)                               //perm:read
    LexGetBlockByNumber(ctx context.Context, blkNum string, fullTxInfo bool) (lextypes.LexBlock, error)                                        //perm:read
    LexGetTransactionByHash(ctx context.Context, txHash *lextypes.lexHash) (*lextypes.LexTx, error)                                            //perm:read
    LexGetTransactionByHashLimited(ctx context.Context, txHash *lextypes.LexHash, limit abi.ChainEpoch) (*lextypes.LexTx, error)               //perm:read
    LexGetTransactionHashByCid(ctx context.Context, cid cid.Cid) (*lextypes.LexHash, error)                                                    //perm:read
    LexGetMessageCidByTransactionHash(ctx context.Context, txHash *lextypes.LexHash) (*cid.Cid, error)                                         //perm:read
    LexGetTransactionCount(ctx context.Context, sender lextypes.LexAddress, blkOpt string) (lextypes.LexUint64, error)                         //perm:read
    LexGetTransactionReceipt(ctx context.Context, txHash lextypes.LexHash) (*LexTxReceipt, error)                                              //perm:read
    LexGetTransactionReceiptLimited(ctx context.Context, txHash lextypes.LexHash, limit abi.ChainEpoch) (*LexTxReceipt, error)                 //perm:read
    LexGetTransactionByBlockHashAndIndex(ctx context.Context, blkHash lextypes.LexHash, txIndex lextypes.LexUint64) (Lextypes.LexTx, error)    //perm:read
    LexGetTransactionByBlockNumberAndIndex(ctx context.Context, blkNum lextypes.LexUint64, txIndex lextypes.LexUint64) (lextypes.LexTx, error) //perm:read

    LexGetCode(ctx context.Context, address lextypes.LexAddress, blkOpt string) (lextypes.LexBytes, error)                                    //perm:read
    LexGetStorageAt(ctx context.Context, address lextypes.LexAddress, position lextypes.LexBytes, blkParam string) (lextypes.LexBytes, error) //perm:read
    LexGetBalance(ctx context.Context, address lextypes.LexAddress, blkParam string) (lextypes.LexBigInt, error)                              //perm:read
    LexChainId(ctx context.Context) (lextypes.LexUint64, error)                                                                               //perm:read
    LexSyncing(ctx context.Context) (lextypes.LexSyncingResult, error)                                                                        //perm:read
    LexVersion(ctx context.Context) (string, error)                                                                                           //perm:read
    NetListening(ctx context.Context) (bool, error)                                                                                           //perm:read
    LexProtocolVersion(ctx context.Context) (lextypes.LexUint64, error)                                                                       //perm:read
    LexGasPrice(ctx context.Context) (lextypes.LexBigInt, error)                                                                              //perm:read
    LexFeeHistory(ctx context.Context, p jsonrpc.RawParams) (lextypes.lexFeeHistory, error)                                                   //perm:read

    LexMaxPriorityFeePerGas(ctx context.Context) (lextypes.lexBigInt, error)                      //perm:read
    LexEstimateGas(ctx context.Context, tx lextypes.lexCall) (lextypes.lexUint64, error)          //perm:read
    LexCall(ctx context.Context, tx lextypes.lexCall, blkParam string) (lextypes.lexBytes, error) //perm:read

    lexSendRawTransaction(ctx context.Context, rawTx lextypes.lexBytes) (lextypes.lexHash, error) //perm:read

    // Returns event logs matching given filter spec.
    lexGetLogs(ctx context.Context, filter *lextypes.lexFilterSpec) (*lextypes.lexFilterResult, error) //perm:read

    // Polling method for a filter, returns event logs which occurred since last poll.
    // (requires write perm since timestamp of last filter execution will be written)
    lexGetFilterChanges(ctx context.Context, id lextypes.lexFilterID) (*lextypes.lexFilterResult, error) //perm:read

    // Returns event logs matching filter with given id.
    // (requires write perm since timestamp of last filter execution will be written)
    LexGetFilterLogs(ctx context.Context, id lextypes.lexFilterID) (*lextypes.lexFilterResult, error) //perm:read

    // Installs a persistent filter based on given filter spec.
    LexNewFilter(ctx context.Context, filter *lextypes.lexFilterSpec) (lextypes.lexFilterID, error) //perm:read

    // Installs a persistent filter to notify when a new block arrives.
    LexNewBlockFilter(ctx context.Context) (lextypes.lexFilterID, error) //perm:read

    // Installs a persistent filter to notify when new messages arrive in the message pool.
    LexNewPendingTransactionFilter(ctx context.Context) (lextypes.lexFilterID, error) //perm:read

    // Uninstalls a filter with given id.
    LexUninstallFilter(ctx context.Context, id lextypes.lexFilterID) (bool, error) //perm:read
    ...
    // The client will receive a stream of lexSubscriptionResponse values until lexUnsubscribe is called.
    LexSubscribe(ctx context.Context, params jsonrpc.RawParams) (lextypes.lexSubscriptionID, error) //perm:read

    // Unsubscribe from a websocket subscription
    LexUnsubscribe(ctx context.Context, id lextypes.lexSubscriptionID) (bool, error) //perm:read
...
// reverse interface to the client, called after LexSubscribe
type LexSubscriber interface {
    // note: the parameter is lextypes.lexSubscriptionResponse serialized as json object
    lexSubscription(ctx context.Context, r jsonrpc.RawParams) error // rpc_method:lex_subscription notify:true
}
