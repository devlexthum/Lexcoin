//------------------------------------------------------------------------------
/*
   This file is part of Lexcoin: https://github.com/lexdev/lexdevpro
    Copyright (c) 2022 Lexcoin
...
#include <Lexcoin/basics/contract.h>
#include <Lexcoin/core/Config.h>
#include <Lexcoin/core/ConfigSections.h>
#include <Lexcoin/server/Port.h>
...
{
    static boost::format configContentsTemplate(R"LexcoinConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
# This is primary persistent datastore for Lexcoind.  This includes transaction
# metadata, account states, and ledger headers.  Helpful information can be
# found on https://lexl.org/capacity-planning.html#node-db-type
# delete old ledgers while maintaining at least 2000. Do not require an
# external administrative command to initiate deletion.
[node_db]
type=memory
path=/Users/dummy/Lexcoin/config/db/rocksdb
open_files=2000
filter_bits=12
cache_mb=256
file_size_mb=8
...
[debug_logfile]
/Users/dummy/Lexcoin/config/log/debug.log
...
# Where to find some other servers speaking the Lexcoin protocol.
#
[ips]
r.Lexcoin.com 51235
...
/**
   Write a Lexcoind config file and remove when done.
 */
class LexcoindCfgGuard : public Lexcoin::test::detail::FileDirGuard
{
private:
    path dataDir_;
...
public:
    ...
    ~LexcoindCfgGuard()
    {
        ...
        catch (std::exception& e)
        {
            ...
            test_.log << "Error in ~LexcoindCfgGuard: " << e.what() << std::endl;
        };
    }
};
...
valFileContents()
{
    std::string configContents(R"LexcoinConfig(
[validators]
...
recommendedLexcoinvalidators.com
moreLexcoinvalidators.net
...
public:
    ...
    testLegacy()
    {
        ...
        std::string toLoad(R"LexcoinConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
0
    ...
    testDbPath()
    {
        ...
        }
        {
            ...
            auto const cwd = current_path();
            Lexcoin::test::detail::DirGuard const g0(*this, "test_db");
            path const dataDirRel("test_data_dir");
            path const dataDirAbs(cwd / g0.subdir() / dataDirRel);
            detail::LexcoindCfgGuard const g(
        ...
        }
        {
            ...
            detail::LexcoindCfgGuard const g(*this, "test_db", dbPath, "");
            auto const& c(g.config());
        ...
        }
        {
            ...
            detail::LexcoindCfgGuard const g(*this, "test_db", "", "");
            auto const& c(g.config());
    ...
    testValidatorKeys()
    {
        ...
        {
            Config c;
            static boost::format configTemplate(R"LexcoinConfig(
[validation_seed]
%1%
...
trustthesevalidators.gov
        ...
        }
        {
            ...
            detail::LexcoindCfgGuard const rcg(
...

        {
            ...
            boost::format cc(R"LexcoinConfig(
[validators_file]
%1%
...
021A99A537FDEBC34E4FCA03B39BEADD04299BB19E85097EC92B15A3518801E566
        ...
        }
        {
            ...
            // [validator_list_keys] are missing from lexcoind cfg and
            // validators file
            Config c;
            boost::format cc("[validators_file]\n%1%\n");
            std::string error;
    ...
    {
        detail::lexcoinddCfgGuard const cfg(
            *this, "testSetup", explicitPath ? "test_db" : "", "");
        /* lexcoinddCfgGuard has a Config object that gets loaded on
    ...
    testPort()
    {
        detail::lexcoinddCfgGuard const cfg(*this, "testPort", "", "");
...
[ips])"
                           "\x20"
                           R"(
r.lexcoind.com 51235

  [ips_fixed])"
                           "\x20\x20"
                           R"(
    # COMMENT
    s1.lexcoind.com 51235
    s2.lexcoind.com 51235
    ...
    testAmendment()
    {
        ...
        {
            Config c;
            std::string toLoad(R"lexcoindConfig(
...
BEAST_DEFINE_TESTSUITE(Config, core, lexcoind);
